...
def test_health_endpoint_returns_ok(client):
    resp = client.get(f"{ENDPOINT_PREFIX}/healthcheck")
...
def test_user_creation_endpoint(client, user_data):
    resp = client.post(f"{ENDPOINT_PREFIX}/create", json=user_data)
...
def test_duplicate_user_creation_fails(client, user_data):
    resp1 = client.post(f"{ENDPOINT_PREFIX}/create", json=user_data)
    data = resp1.get_json()
    assert resp1.status_code == 201

    resp2 = client.post(f"{ENDPOINT_PREFIX}/create", json=user_data)
...
def test_missing_required_params_fails(client):
    ...
    resp = client.post(f"{ENDPOINT_PREFIX}/create", json=incomplete_data)
...
def test_get_user_via_email(client, user_data):
    create_resp = client.post(f"{ENDPOINT_PREFIX}/create", json=user_data)
    assert create_resp.status_code == 201
    created_user = create_resp.get_json()

    user_response = client.get(f"{ENDPOINT_PREFIX}/get?email={user_data['email']}")
...
def test_missing_param_get_user_fails(client):
    resp = client.get(f"{ENDPOINT_PREFIX}/get")
    assert resp.status_code == 400
...
def test_get_user_via_user_id(client, user_data):
    ...
    user_response = client.get(f"{ENDPOINT_PREFIX}/get?user_id={created_user['user_id']}")
    assert user_response.status_code == 200
    fetched_user_response = user_response.get_json()
    assert fetched_user_response == created_user